...
export class AuditLogger {
  ...
  private async persistLog(entry: AuditLogEntry): Promise<void> {
    switch (this.config.destination) {
      case 'database':
        // Save directly to Prisma database
        try {
          ...
          await prisma.auditLog.create({
            data: {
              ...
              action: `security2.${entry.action}`,
              resource: entry.resource,
              target: entry.resourceId || null,
              decision: entry.decision,
              reason: entry.reason || null,
              meta: {
                principalRoles: entry.principalRoles,
                matchedRules: entry.matchedRules,
                derivedRoles: entry.derivedRoles,
      ...
      case 'opensearch':
        ...
        break
      default:
        // Default: console log only in development
        if (process.env.NODE_ENV === 'development') {
          console.log('[AUDIT]', JSON.stringify(entry))
        }
    }
