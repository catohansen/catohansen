...
import { SecuritySettingsManager } from './SecuritySettings'
...
export class HansenSecurityPolicyEngine {
  ...
  async evaluate(
    ...
    try {
      // DEVELOPMENT MODE: Bypass all security if securityEnabled is false
      ...
      if (!settings.securityEnabled && process.env.NODE_ENV === 'development') {
        ...
        await auditLogger.logDecision({
          ...
          decision: 'ALLOW',
          effect: 'ALLOW',
          reason: 'Development mode: Security disabled',
          matchedRules: ['DEV_MODE_BYPASS'],
          latencyMs,
          correlationId
        })

        metricsCollector.recordDecision('ALLOW', latencyMs, 'DEV_MODE', resource.kind)
