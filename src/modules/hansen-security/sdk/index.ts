...
/**
 * Hansen Security Client
 * Client for checking permissions via API
 */
export class HansenSecurityClient {
  constructor(private baseUrl: string = '/api/modules/hansen-security') {}
  ...
  async check(
    ...
    try {
      ...
      console.error('Hansen Security check failed:', error)
      return {
        allowed: false,
  ...
  async checkMultiple(
    ...
    try {
      ...
      console.error('Hansen Security checkMultiple failed:', error)
